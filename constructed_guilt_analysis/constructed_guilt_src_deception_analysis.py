...
def plot_feature_distributions(df, out_dir=None):
...
def plot_effect_sizes(comp_df, out_dir=None):
...
def plot_hedge_certainty_scatter(df, out_dir=None):
    """
...
def save_spss_csv(df, path=None):
    """Save numeric columns in SPSS-importable format."""
    if path is None:
        path = os.path.join(PROJECT_ROOT, "results", "spss_ready.csv")
...
def save_results_table(comp_df, corr_df, df, path=None):
    """Write formatted results tables to text file."""
    if path is None:
        path = os.path.join(PROJECT_ROOT, "results", "tables.txt")
