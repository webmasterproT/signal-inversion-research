...
os.makedirs("results/cultural", exist_ok=True)
...
def load_open_domain(base_dir="data/openDomain"):
    """
...
def load_cross_cultural(base_dir="data/crossCultural"):
    """
    Expected layout:
        data/crossCultural/
            US/
...
def plot_cultural_boxplots(df, dataset_name, out_prefix):
    ...
    plt.tight_layout()
    path = f"results/figures/{out_prefix}_cultural_boxplots.png"
...
def plot_cross_classifier_error(df, out_prefix):
    ...
    plt.tight_layout()
    path = f"results/figures/{out_prefix}_cross_classifier_fp.png"
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  Saved: {path}")
...
def main():
    ...
    print("\n── Dataset 1: Open-Domain Deception ────────────────────────")
    od_df = load_open_domain("data/openDomain")
    ...
    print("\n── Dataset 2: Cross-Cultural Deception ─────────────────────")
    _cc_paths = ["../cultural_heuristics/crossCulturalDeception.2014", "data/crossCultural"]
    cc_df = pd.DataFrame()
    for _path in _cc_paths:
        if os.path.isdir(_path):
            cc_df = load_cross_cultural(_path)
            if not cc_df.empty:
                break
    ...
    # ── Save results ──────────────────────────────────────────────────
    print("\n── Saving results ───────────────────────────────────────────")
    with open("results/cultural/cultural_analysis.txt", "w") as f:
        f.write("=" * 70 + "\n")
    ...
    print("  Saved: results/cultural/cultural_analysis.txt")
    ...
    print("\nOutputs:")
    print("  results/cultural/cultural_analysis.txt")
    print("  results/figures/*_cultural_boxplots.png")
    print("  results/figures/*_cross_classifier_fp.png")
    print("=" * 65 + "\n")
