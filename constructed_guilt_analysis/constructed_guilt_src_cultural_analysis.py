"""
Cultural & Demographic Deception Analysis Pipeline
===================================================
...
Datasets used:
    1. Open-Domain Deception (Pérez-Rosas & Mihalcea, 2015)
    ...
    2. Cross-Cultural Deception (Pérez-Rosas & Mihalcea, 2014)
       US, India, Mexico, Romania — essays on abortion, death penalty, best friend
       Already present in data/raw/crossCulturalDeception.2014/
...
Run:
    cd cross_cultural_deception/
    python3 src/cultural_analysis.py
"""
...
def load_open_domain(base_dir=None):
    """
    ...
    """
    if base_dir is None:
        base_dir = os.path.join(PROJECT_ROOT, "data", "openDomain")
...
# Map directory names to culture labels
FOLDER_TO_CULTURE = {
    'englishus':      'US',
    'englishindia':   'India',
    'spanishmexico':  'Mexico',
    'romanian':       'Romania',
}

# Map topic abbreviations and filenames
TOPIC_MAP = {
    'abortion':      'abortion',
...
def load_cross_cultural(base_dir=None):
    """
    Reads the Pérez-Rosas & Mihalcea (2014) cross-cultural deception corpus.

    Actual file format (tab-separated, one essay per line):
        data/raw/crossCulturalDeception.2014/
            EnglishUS/
                ab/  abortion.True, abortion.False
                bf/  bestFriend.True, bestFriend.False
                dp/  deathPenalty.True, deathPenalty.False
            EnglishIndia/  (same structure)
            SpanishMexico/ (same structure)
            Romanian/      (same structure)

    Each file contains lines like:
        abortion.True_1\\t I don't think it's bad. The baby isn't...
    """
    if base_dir is None:
        base_dir = os.path.join(PROJECT_ROOT, "data", "raw", "crossCulturalDeception.2014")
    ...
    # Walk all culture directories
    for culture_dir in sorted(os.listdir(base_dir)):
        culture_path = os.path.join(base_dir, culture_dir)
        if not os.path.isdir(culture_path):
            continue

        culture = FOLDER_TO_CULTURE.get(culture_dir.lower(), None)
        if culture is None:
            continue

        # Walk topic subdirs (ab, bf, dp)
        for topic_dir in sorted(os.listdir(culture_path)):
            topic_path = os.path.join(culture_path, topic_dir)
            ...
            for filename in sorted(os.listdir(topic_path)):
                ...
                # Determine label from filename suffix (.True = truthful, .False = deceptive)
                ...
                topic = TOPIC_MAP.get(base_name, 'unknown')

                # Read file — each line is: id\ttext
                try:
    ...
    for csv_path in glob.glob(os.path.join(base_dir, "**", "*.csv"), recursive=True):
        try:
            ...
            for _, row in df.iterrows():
                ...
                feats['label']     = label
                feats['label_str'] = 'Deceptive' if label == 1 else 'Truthful'
                feats['country']   = str(row[country_col]) if country_col else 'Unknown'
...
def plot_cultural_boxplots(df, dataset_name, out_prefix):
    ...
    plt.tight_layout()
    path = os.path.join(PROJECT_ROOT, f"results/figures/{out_prefix}_cultural_boxplots.png")
...
def plot_cross_classifier_error(df, out_prefix):
    ...
    plt.tight_layout()
    path = os.path.join(PROJECT_ROOT, f"results/figures/{out_prefix}_cross_classifier_fp.png")
    plt.savefig(path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  Saved: {path}")
...
def main():
    print("\n" + "=" * 65)
    print("CULTURAL DECEPTION ANALYSIS — Constructed Guilt Appendix")
    print("=" * 65)
    ...
    if od_df.empty or 'label' not in od_df.columns:
        ...
        if not results.empty:
            ...
            plot_cultural_boxplots(od_df, "Open-Domain Deception (Perez-Rosas & Mihalcea, 2015)", "openDomain")
            plot_cross_classifier_error(od_df, "openDomain")

    # ── Dataset 2: Cross-Cultural ─────────────────────────────────────
    print("\n-- Dataset 2: Cross-Cultural Deception ----------------------")
    cc_df = load_cross_cultural()

    if cc_df.empty or 'label' not in cc_df.columns:
        print("  !!  Cross-Cultural data not found at data/raw/crossCulturalDeception.2014/")
        print("     Download instructions:")
        print("     curl -L 'https://web.eecs.umich.edu/~mihalcea/downloads/crossCulturalDeception.2014.tar.gz' -o crossCultural.tar.gz")
        print("     tar -xzf crossCultural.tar.gz -C data/raw/")
        print("\n  Running on DEMO DATA for pipeline validation...")
        cc_df = generate_demo_cultural()
        print(f"  Demo: {len(cc_df)} synthetic statements")
    else:
        print(f"  Loaded {len(cc_df)} statements from real corpus")

    results = analyse_cultural_variation(cc_df, "Cross-Cultural Deception", "crossCultural")
    if not results.empty:
        all_results['cross_cultural'] = results
        plot_cultural_boxplots(cc_df, "Cross-Cultural Deception (Perez-Rosas & Mihalcea, 2014)", "crossCultural")
        plot_cross_classifier_error(cc_df, "crossCultural")

    # ── Save results ──────────────────────────────────────────────────
    print("\n-- Saving results -------------------------------------------")
    out_path = os.path.join(PROJECT_ROOT, "results/cultural/cultural_analysis.txt")
    with open(out_path, "w") as f:
        f.write("=" * 70 + "\n")
        f.write("CULTURAL VARIATION IN TRUTHFUL SPEECH — RESULTS\n")
        f.write("For thesis: Constructed Guilt — Appendix Section 6\n")
        f.write("=" * 70 + "\n\n")
